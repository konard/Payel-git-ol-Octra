...
(или пусто, если `--prompt-experiments-folder "" --prompt-examples-folder ""`)
...
Если `promptCheckSiblingPullRequests !== false` (по умолчанию true):
```
...
Если `--prompt-general-purpose-sub-agent`:
```
...
## Plan sub-agent (если `--prompt-plan-sub-agent`)
...
## Agent Commander (если `--prompt-subagents-via-agent-commander` и agentCommanderInstalled)
...
Agent Commander usage (unified subagent delegation).
   ...
   - Common start-agent parameters:
      ...
      --model <name>: Model to use (sonnet, opus, haiku, gpt-5.5, nemotron-3-super-free, etc.)
...
## HANDOFF.md (если `--use-handoff`)

```
HANDOFF.md continuity skill (experimental, --use-handoff).
   - A reusable "handoff" Agent Skill is installed in this workspace (.claude/skills/handoff/ for Claude, .agents/skills/handoff/ for Codex). It defines how to read and maintain HANDOFF.md so any session can continue the work of a previous one — even across tools (Claude and Codex) in the same pull request.
   - At the start of this session, use the handoff skill: if HANDOFF.md exists in the repository root, read it first and continue from its "Next steps". Create or update HANDOFF.md as you make progress and commit it to the pull request branch.
```
