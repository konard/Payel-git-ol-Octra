...
// createTaskMD — воркер пишет TASK.md (детальный план) с помощью LLM
func (s *Service) createTaskMD(ctx context.Context, provider, model string, tokens map[string]string, role, description, taskMD, externalContext string) (string, error) {
	contextSection := ""
	if externalContext != "" {
