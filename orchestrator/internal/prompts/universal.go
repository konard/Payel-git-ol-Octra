...
//
// Когда модель оценивает задачу как тривиальную, нет смысла гонять её через весь
// конвейер Boss → Manager → Worker и плодить ноды. Универсальная нода — это один
// AI-воркер с менее ограниченными возможностями: она читает задачу и сразу выдаёт
// минимальный корректный результат. Главное правило — НЕ переусложнять:
