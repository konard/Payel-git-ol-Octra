...
func TestRegisterAndAuthenticate(t *testing.T) {
	...
	svc := NewAuthService(users, testCfg(), transactions)
...
func seedUser(t *testing.T, db *gorm.DB) *model.User {
	t.Helper()
	u, err := NewAuthService(repository.NewUserRepository(db), testCfg(), repository.NewTransactionRepository(db)).Register(context.Background(), "seeduser", "u@e.com", "pw")
	if err != nil {
		t.Fatal(err)
	}
	return u
